%% ss23: single-cell vs replicate average trend of slope_i vs Vb


%  Goal: compare individual and mean data in slope_i and Vb
...
%  Commit: first commit, plots spread of individual slope_i vs Vb with averaged
...
for ii = 1:length(steady_Vb.mean)
...
clear ts_Vb ts_slope s_std v_std ii num_ii vb_ii slope_ii
...
clear mean_slope_steady mean_Vb_steady std_slope_steady std_Vb_steady 
clear numcells_steady sem_slope_steady sem_Vb_steady
clear mean_slope_fluc mean_Vb_fluc std_slope_fluc std_Vb_fluc sem_slope_fluc sem_Vb_fluc
...
clear col fluc_Vb_i fluc_slope_i steady_Vb_i steady_slope_i
clear fluc_Vb_i_restruct fluc_slope_i_restruct fluc_lambda_i fluc_slopes_i
clear steady_Vb steady_slope fluc_slope fluc_Vb
...
mean_slope = cellfun(@mean,slope_i_data);
mean_Vb = cellfun(@mean,Vb_i_data);
std_slope = cellfun(@std,slope_i_data);
std_Vb = cellfun(@std,Vb_i_data);
n_Vb = cellfun(@length,Vb_i_data);
sem_slope = std_slope./sqrt(n_Vb);
sem_Vb = std_Vb./sqrt(n_Vb);


% 2. loop through experimental replicates
for rr = 1:length(slope_i_data)
    
    clear curr_tau curr_Vb
    
    rep_slope = slope_i_data(rr,:);
    if isempty(rep_slope{1}) == 1
        continue
    end
    ...
    for cond = 1:length(rep_slope)
        
        curr_slope = rep_slope{cond};
        curr_Vb = rep_Vb{cond};
        curr_color = rgb(palette_i{cond});
        
        figure(counter)
        scatter(curr_Vb,curr_slope,'MarkerFaceColor',curr_color,'MarkerEdgeColor',curr_color)
        hold on
        
    end
    clear cond curr_slope curr_Vb curr_color rep_Vb rep_slope
...
cd('/Users/jen/Documents/StockerLab/Writing/manuscript 2/superSize_figs/ss23/')
for rep = 1:13
    figure(rep)
    saveas(gcf,strcat('ss23-indi-v-rep-',num2str(rep)),'epsc')
    close(gcf)
end
