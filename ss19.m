%% ss19: trends in single cell and replicate log_10(Vb) vs lambda


%  Goal: compare trend between replicates with trend within a replicate
%        just like ss18.m except plotting on a log scale
...
%  Commit: first commit, single cell vs average trends in log_10(Vb) vs lambda
...
%     yerr = sem_Vb_steady(:,sc);
    ...
    scatter(xvals,log10(yvals),100,'filled','MarkerFaceColor',sc_color)
    hold on
...
%     yerr = sem_Vb_fluc(:,ts);
    ...
    % iii. plot condition data
    figure(1) % slope vs Vb (mean +/- sem)
    scatter(xvals,log10(yvals),100,'filled','MarkerFaceColor',color)
    hold on
...
title('semilog mean Vb_i vs. mean lambda of each replicate')
axis([0 4 0 1.2])
...
cd('/Users/jen/Documents/StockerLab/Writing/manuscript 2/superSize_figs/ss19/')
counter = 0;
...
for condition = 1:3
    for row = 1:length(steady_Vb_i)
        ...
        if isempty(Vb_i) == 1
            ...
            figure(2)
            scatter(lambda_i,log10(Vb_i),10,'MarkerEdgeColor',rgb('Silver'))
            hold on
            ...
            scatter(rc_mean_gr,log10(rc_mean_vb),100,'filled','MarkerFaceColor',rc_color)
            hold on
...
%                 
                % iii. plot condition data
                figure(2) % slope vs Vb (mean +/- sem)
                scatter(xvals,log10(yvals),100,'MarkerEdgeColor',sc_color)
                hold on
%                 errorbar(xvals,yvals,xerr,'.','horizontal','Color',sc_color)
%                 hold on
%                 errorbar(xvals,yvals,yerr,'.','Color',sc_color)
%                 hold on
            ...
            figure(2)
            saveas(gcf,strcat('ss19-rc-',num2str(counter)),'epsc')
            close(gcf)
        end
...
for timescale = 1:4
    for rw = 1:length(fluc_lambda_i)
        ...
        if isempty(Vb_if) == 1
            ...
            figure(3)
            scatter(lambda_if,log10(Vb_if),10,'MarkerEdgeColor',rgb('Silver'))
            hold on
            ...
            scatter(rt_mean_gr,log10(rt_mean_vb),100,'filled','MarkerFaceColor',rt_color)
            hold on
...
%                 yerr = sem_Vb_fluc(:,fl);
                ...
                % iii. plot condition data
                figure(3) % slope vs Vb (mean +/- sem)
                scatter(xvals,log10(yvals),100,'MarkerEdgeColor',fl_color)
                hold on
