%% Figure 2D: fluctuating (T = 15min) motion relative to steady relationship
...
%  Commit: first commit, 15 min period, timestep: 5 min
...
save('B4_ccSize.mat','compiled_data','compiled_mu','exptArray')
...
shiftFactor = 10;
timescale = 900/3600; % as a fraction of an hour
binsPerPeriod = binsPerHour*timescale;


% time bins of interest in this analysis 
minBin = binsPerPeriod*shiftFactor; 
...
for metric = 1:length(sm)
    ...
    for jj = 1:length(conditions) % condition
        ...
        % FRACTION
        cond_time_in_periods = cond_times/timescale;
        cond_fractions = cond_time_in_periods - floor(cond_time_in_periods); % of period
        cond_fractions_shifted = cond_fractions + shiftFactor;
        bins_birth = ceil(cond_fractions_shifted * binsPerPeriod);
        ...
        timeVector_BT = ((1:length(volBirth_binnedByBT))/binsPerPeriod) - shiftFactor;
