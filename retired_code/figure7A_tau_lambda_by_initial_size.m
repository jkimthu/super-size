%% Figure 7A: tau and lambda as a function of initial size
...
%  Commit: edit figure 3A to focus on tau and lambda
...
%% Part 3. store birth size in similar variable
...
for condition = 1:length(environment_order)
    ...
    interdiv_min = metas{1,condition}(:,tau);
    interdiv_h = interdiv_min/60;
    ...
    tau_binned = accumarray(birth_bin,interdiv_h,[],@(x) {x});
    ...
    %hold on
    errorbar(tau_mean,tau_sem,'.','Color',rgb('SlateGray'))
    hold on
    plot(1./gr_mean,'o','Color',rgb(palette{condition}))

    
    ylabel('inverse lambda')
    ...
    %axis([3 53 0 7])
    %axis([3 53 -5 7])
    legend('measured tau','expected tau')
