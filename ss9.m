%% ss9: does tau_i(Vb_i) predict Vb_mean?


%  Goal: can we predict V_mean from mu_mean and linear fit of
%        tau_i and Vb_i? 
...
%  Last edit: Jen Nguyen, 2020 Mar 15
%  Commit: building from ss8 to test if V_mean = (yint-1/mu_mean)/slope
...
for cond = 1:length(conditionsOI)
    ...
    for rep = 1:length(currData)
        ...
        % v. save fit for comparing predicted and measured values
        conditions(counter) = currCond;
        slopes(counter) = fit(1);
        yints(counter) = fit(2);
        Vb_means(counter) = Vb_mean;
        lambda_inverse_means(counter) = mean(gr_inverse_trim2);
