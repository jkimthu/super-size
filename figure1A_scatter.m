...
%  Last edit: Jen Nguyen, 2019 Feb 3
%  Commit: scatter size vs growth rate, colored by condition
...
%% Part 3. sort data by nutrient condition, keeping replicates apart
...
%% Part 4. plot scatter of each fluctuating replicate per condition

fluctuating = [2,3,4,5];
steady = [1,6,7];
metric = 1; % 1 = volume at birth
r_color = {'DarkSlateBlue','MediumSlateBlue','DarkOrchid','Plum',...
           'CadetBlue','SteelBlue','DarkCyan','OliveDrab','DarkGreen'...
           'GoldenRod','BurlyWood','Peru','MediumVioletRed'};

for nc = 1:length(environment_order) % nc = nutrient condition
    
    figure(nc)
    current_data = organized_data(:,nc);
    
    % isolate data from each replicate
    for rep = 1:length(current_data)
        
        rep_data = current_data{rep,1};
        if isempty(rep_data) == 1
            continue
        end
        lambda = rep_data.meta(:,lamb); % note: mu is all instananeous vals in each cell cycle
        sizes = rep_data.cc(:,metric);
        %plus = current_data.plus1;
        
        hold on
        scatter(lambda,sizes,'filled','MarkerFaceColor',rgb(r_color{rep}))
...
%% Part 5. plot replicate means of steady, scatter of fluctuating


fluctuating = [2,3,4,5];
steady = [1,6,7];
metric = 1; % 1 = volume at birth
colors = {'Indigo','CadetBlue','SteelBlue','DarkCyan','OliveDrab','GoldenRod','FireBrick'};
...
for nc = 1:length(environment_order) % nc = nutrient condition
    ...
    % for steady environments, plot mean and sem of each replicate
    if ismember(nc,steady)
        ...
        for rep = 1:length(current_data)
            ...
            figure(1)
            hold on
            plot(mean(lambda),mean(sizes),'o','Color',rgb(colors{nc}))

        end
        
    else % fluctuating condition! 
         % plot scatter of each cell from each replicate (distinguished by transparency)
        ...
        for rep = 1:length(current_data)
            ...
            hold on
            scatter(lambda,sizes,'filled','MarkerFaceColor',rgb(colors{nc}))
            hold off
            alpha(1/r_counter)
        end
